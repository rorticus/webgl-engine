...
void main() {
  mat4 skinMatrix = getBoneMatrix(a_joints_0[0]) * a_weights_0[0] + 
    ...
    // mat4 skinMatrix = u_bones[int(a_joints_0[0])] * a_weights_0[0] +
    //     u_bones[int(a_joints_0[1])] * a_weights_0[1] +
    //     u_bones[int(a_joints_0[2])] * a_weights_0[2] +
    //     u_bones[int(a_joints_0[3])] * a_weights_0[3];
...
vec3 calculatePositionalLights(vec3 normal) {
	...
	for(int i = 0; i < NUM_POSITIONAL_LIGHTS; i++) {
		...
		diffuse += u_lightWorldColor[i] * light;				
	}
